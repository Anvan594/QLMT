...
def init_db():
    ...
    conn.close()
def seed(conn):
    dept_seed = [('Văn phòng', ['Nhân sự', 'Kế Toán','Mua Bán','Xuất Nhập Khẩu','Kinh Doanh','QC']), ('Kho', ['Kho','QLSX']), ('Xưởng', ['Thiết Bị','Nhựa','Kim Loại'])]
    dept_ids, pos_ids = ({}, {})
...
def device_to_dict(r):
    keys = set(r.keys()) if hasattr(r, 'keys') else []
    def g(k, default=''):
        return r[k] if k in keys and r[k] is not None else default

    # Tự động tra cứu tên người dùng dựa vào userId nếu có
    user_name = ''
    user_id_val = g('user_id')
    if user_id_val:
        try:
            conn_temp = get_db()
            u_row = conn_temp.execute('SELECT full_name FROM users WHERE id=?', (user_id_val,)).fetchone()
            if u_row:
    ...
    return {
        ...
        'macPhysical': g('mac_physical'), 'macVirtual': g('mac_virtual'), 
        'userId': user_id_val,
        'userName': user_name  # Trả về tên người dùng để giao diện hiển thị lên bảng
    }
...
def user_to_dict(r):
    ...
    # Lấy tên phòng ban hoặc vị trí để hiển thị kèm theo tên người dùng cho khớp với giao diện form thiết bị
    dept_name = ''
    try:
        if 'dept_id' in keys and r['dept_id']:
            conn_temp = get_db()
            d_row = conn_temp.execute('SELECT name FROM departments WHERE id=?', (r['dept_id'],)).fetchone()
            if d_row:
    ...
    full_name = r['full_name'] if ('full_name' in keys and r['full_name']) else ''
    position_val = r['position'] if ('position' in keys and r['position']) else ''

    return {
        'id': r['id'], 
        'fullName': full_name, 
        'name': full_name, # Hỗ trợ cả key 'name' phòng hờ giao diện gọi kiểu cũ
        'email': r['email'] if 'email' in keys else '', 
        'phone': r['phone'] if 'phone' in keys else '', 
        'deptId': r['dept_id'] if 'dept_id' in keys else '', 
        'posId': r['pos_id'] if 'pos_id' in keys else '', 
        'position': position_val, 
        'departmentName': dept_name, # Gửi kèm tên phòng ban để form thiết bị ghép chuỗi "Tên - Phòng"
        'note': r['note'] if 'note' in keys else '', 
        'createdAt': r['created_at'] if 'created_at' in keys else 0
    }
...
def r_devices_import_template(m, body, qs):
    ...
    try:
        ...
        # Đã lược bỏ hoàn toàn các cột theo yêu cầu: ngày nhập, ngày phân, bảo hành, nhà cung cấp, giá trị, main, key win, system, serial, mac
        headers = [
            'Mã máy tính', 'Phòng/Ban *', 'Vị trí cụ thể *', 'Nhóm thiết bị *', 'Loại thiết bị *', 
            'Model', 'Hãng sản xuất', 'Cấu hình/Mô tả', 'Tình trạng', 'Người sử dụng', 'Ghi chú',
            'CPU', 'RAM', 'GPU', 'Ổ cứng', 'Hệ điều hành'
        ]
        
        example = [
            'QN001', dept0['name'] if dept0 else 'Kho', pos0['name'] if pos0 else 'Kho', 
            grp0['name'] if grp0 else 'Thiết bị CNTT', typ0['name'] if typ0 else 'Máy tính', 
            'Latitude 5510', 'Dell Inc.', 'Core i5/24GB', 'Bình thường', '', '',
            'Intel Core i5-10310U', '24 GB', 'Intel UHD Graphics', '512GB SSD', 'Windows 10 Pro'
        ]
...
def r_devices_import(m, body, qs):
    ...
    try:
        ...
        for n, row in enumerate(rows[1:], 2):
            ...
            conn.execute(
                '''INSERT INTO devices(
                    id, asset_code, dept_id, pos_id, group_id, type_id, model, manufacturer, serial, 
                    config, status, import_date, allocate_date, warranty_months, warranty_unit, 
                    supplier, value, note, created_at, user_id, 
                    computer_name, mainboard, cpu, ram, gpu, storage, os_name, 
                ...
                (
                    ...
                    get_val(row, 'Mã máy tính'), get_val(row, 'Mainboard'), get_val(row, 'CPU'), 
                    get_val(row, 'RAM'), get_val(row, 'GPU'), get_val(row, 'Bộ nhớ lưu trữ (SSD/HDD)'), 
                    get_val(row, 'Hệ điều hành'), get_val(row, 'Key Windows (BIOS)'), 
                    get_val(row, 'Key Windows (Hiện tại)'), get_val(row, 'System Model'), 
                    get_val(row, 'Số Serial Number'), get_val(row, 'Địa chỉ MAC Vật lý'), 
                    get_val(row, 'Địa chỉ MAC Ẩo')
                )
            )
            if serial: 
...
def r_devices_update(m, body, qs):
    ...
    try:
        ...
        if serial_val and conn.execute('SELECT 1 FROM devices WHERE id!=? AND LOWER(TRIM(serial))=LOWER(?)',(did,serial_val)).fetchone(): 
            return err(f'Số Serial "{serial_val}" đã tồn tại trên thiết bị khác!')
            
        # Thêm 'user_id' vào danh sách các trường được phép cập nhật
        ...
        # Đảm bảo ánh xạ đúng key 'userId' từ giao diện gửi lên xuống cột 'user_id' trong database
        key_map = {'dept_id':'deptId', 'pos_id':'posId', 'group_id':'groupId', 'type_id':'typeId', 'user_id':'userId'}
        vals = []
        for f in fields:
            body_key = key_map.get(f, f)
            vals.append(body.get(body_key, ''))
            
        vals[14]=_parse_number_cell(str(body.get('value','')))
        vals.append(did)
        
        conn.execute('UPDATE devices SET '+','.join(f+'=?' for f in fields)+' WHERE id=?', vals)
        conn.commit()
        
        r=conn.execute('SELECT * FROM devices WHERE id=?',(did,)).fetchone()
        return (200,device_to_dict(r)) if r else err('Không tìm thấy thiết bị',404)
    except Exception as e: 
        conn.rollback(); 
        return err(f'Lỗi cập nhật thiết bị: {e}',500)
    finally: 
        conn.close()
...
def r_users_create(m, body, qs):
    ...
    if not name:
        ...
        conn = get_db()
        uid = new_id()
        # Lấy chính xác các trường position và posId từ giao diện gửi lên
        conn.execute('INSERT INTO users(id,full_name,email,phone,dept_id,pos_id,position,note,created_at) VALUES(?,?,?,?,?,?,?,?,?)', 
                     (uid, name, body.get('email', '').strip(), body.get('phone', '').strip(), 
                      body.get('deptId', ''), body.get('posId', '').strip(), 
                      body.get('position', '').strip(), body.get('note', '').strip(), now_ms()))
...
def r_users_update(m, body, qs):
    ...
    # Lấy thông tin từ mọi biến giao diện có thể gửi lên để tránh lệch dữ liệu
    ...
    if not name:
        ...
        dept_id = (body.get('deptId') or body.get('department_id') or '').strip()
        pos_id = (body.get('posId') or body.get('position_id') or '').strip()
        position = (body.get('position') or body.get('title') or '').strip()
        note = (body.get('note') or '').strip()

        conn.execute(
            'UPDATE users SET full_name=?, email=?, phone=?, dept_id=?, pos_id=?, position=?, note=? WHERE id=?',
            (name, email, phone, dept_id, pos_id, position, note, uid)
        )
